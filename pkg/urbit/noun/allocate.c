...
#define _box_vaal(box_u)                                        \
  do {                                                          \
  c3_dessert(((uintptr_t)(u3a_box*)(box_u)                      \
              & u3a_balign-1) == 0);                            \
...
_ca_willoc(c3_w len_w, c3_w ald_w, c3_w alp_w)
{
  ...
  //  XX: this logic is totally bizarre, but preserve it. "If neither smallest
  //  (6 words) nor largest (4G), then use the next"
  //
  //  This doesn't lead to fragmentation since these aren't fixed sized
  //  pools. But you should really figure out why this needs to be here in the
  //  first place.
  //
