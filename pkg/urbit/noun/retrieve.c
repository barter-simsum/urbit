...
u3r_met(c3_y  a_y,
        u3_atom b)
{
  c3_dessert(u3_none != b);     /* ;;: can these not be debug asserts? */
  ...
  */
  ...
  /* 5 because 1<<2 bytes in c3_w, 1<<3 bits in byte.
     ...
     gal < 5 informs whether we shift return left or right
     */
  if (a_y < 5) {
    const c3_y max_y = (1 << a_y) - 1;
    if (gal_w > ((UINT32_MAX - (32 + max_y)) >> (5 - a_y)))
  ...
  c3_y gow_y = (a_y - 5);
  return gal_w + 1 >> gow_y; /* + 1 to include daz_w */
}
